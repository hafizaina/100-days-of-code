...
### Day 11: May 19, 2017 - Modals, Navigation, and Segues

**Today's Progress**: In order to navigate between pages, you need to wrap your ViewController inside of a NavigationController. And so begins today's exercise. I started by wrapping my table view controller inside of a navigation controller via Interface Builder. This generates a navigation bar on top of your view, provided by the navigation controller. Next, I configured the navigation bar, adding a title, and some navigation Bar Buttons via the Object Library. To configure the nav button(s), I had to control drag from the button to the appropriate screen, selecting to present the screen modally (meaning the view controller doesn't get added to the navigation stack). Next, I created an unwind segue. An unwind segue moves backward through one or more segues to return the user to a scene managed by an existing view controller. While regular segues create a new instance of the destination view controller, unwind segues let you return to view controllers that already exist. When a segue gets triggered, it provides a place for you to add your own code that gets executed. This method is called prepare(for:sender), and it gives you a chance to store data and do any necessary cleanup on the source view controller (the view controller that the segue is coming from). Next, we have to add an action method to the destination view controller (the view controller the segue is going to). This method must be marked with the IBAction attribute and take a segue (UIStoryboardSegue) as a parameter. In this method ( with signature: 
@IBAction func unwindToMealList(sender: UIStoryboardSegue) ), we wrote logic to add the new meal to the meal list data and add a new row to the table view in the meal list scene. Finally, we implemented the Cancel functionality by making connections to code via Interface Builder, which helped us implement an action listner for the 'Cancel' Bar Button.
